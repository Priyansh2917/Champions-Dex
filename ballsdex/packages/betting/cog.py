...
class BetView(discord.ui.View):
    def __init__(self, cog: "BettingCog", channel_id: int, p1: discord.User, p2: discord.User):
        ...
        self.state = "OPEN"  # OPEN -> CONFIRMING -> FINISHED
        self.p1 = p1
        self.p2 = p2
        ...
        for player in [self.p1, self.p2]:
            ...
            if not pool:
                ...
                for ball in pool:
                    ...
                    lines.append(f"• {emoji} #{ball.pk:0X} {ball.countryball.country}")
                value = "\n".join(lines)
                if len(value) > 1024:
                    value = value[:1000] + "\n*...and more*"
    ...
    async def lock_bet_button(self, interaction: discord.Interaction):
        ...
        if len(self.locked_by) == 2:
            ...
            await interaction.followup.send(
                "Both players have locked their bet. Please tick to confirm the bet.", ephemeral=True
            )
        else:
            await interaction.followup.send(
                "Your proposal has been locked. Waiting for the other user.", ephemeral=True
            )
    ...
    async def cancel_bet_button(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(
            f"The bet was cancelled by <@{interaction.user.id}> and items have been returned.",
            ephemeral=False,
        )
        await self.update_message()
...
class BettingCog(commands.GroupCog, group_name="bet"):
    """
    Commands to bet champions in a betting pool
    """
    ...
    async def begin(self, interaction: discord.Interaction, user: discord.User):
        ...
        if channel_id in self.active_bets:
            return await interaction.followup.send(
                "A bet is already active in this channel. Conclude or cancel it first.", ephemeral=True
            )
        ...
        msg = await interaction.followup.send(
            f"A new betting pool has started between {interaction.user.mention} and {user.mention}!\n"
            f"Use `/bet add` or `/bet bulk_add` to add items to your side.",
            view=view,
            wait=True,
        )
        view.message = msg
    ...
    async def add(self, interaction: discord.Interaction, countryball: BallInstanceTransform):
        """Add a champion to the current bet"""
        await interaction.response.defer(ephemeral=True)
        channel_id = interaction.channel.id
        ...
        if view.state != "OPEN":
            return await interaction.followup.send(
                "The bet is no longer open for additions (items are locked).", ephemeral=True
            )
        ...
        if await countryball.is_locked():
            return await interaction.followup.send(
                f"This {settings.collectible_name} is locked in another trade/bet.", ephemeral=True
            )
        ...
        await interaction.followup.send(
            f"Added #{countryball.pk:0X} {countryball.countryball.country} to the bet.", ephemeral=True
        )
        await view.update_message()
